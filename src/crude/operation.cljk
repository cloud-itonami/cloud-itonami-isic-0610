(ns crude.operation
  "OperationActor -- one crude-petroleum operation = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor (CrudeAdvisor)
  is sealed into a single node (:advise); its proposal is ALWAYS routed
  through the Well Safety Governor (:govern) and the rollout phase gate
  ...
  One graph run = one crude operation (intake -> advise -> govern ->
  decide -> commit | hold | approval). No unbounded inner loop -- each
  operation is auditable and checkpointed. A well's life is advanced by
  MANY ops (assess / lift / settle), each its own independent run.

  Human-in-the-loop = real approval workflow:
  `interrupt-before #{:request-approval}` pauses the actor and hands the
  decision to a human operator (well operator / production superintendent).
  The approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:well/lift`/`:production/settle` ALWAYS reach this node
  when the governor is clean -- see `crude.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [crude.crudeadvisor :as crudeadvisor]
            [crude.governor :as governor]
            [crude.phase :as phase]
...
(defn build
  ...
  `crude.store/Store`).
  opts:
    :advisor      -- a `crude.crudeadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (crudeadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; CrudeAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (crudeadvisor/-advise advisor store request)]
            {:proposal p :audit [(crudeadvisor/trace request p)]})))

      ;; Well Safety Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
