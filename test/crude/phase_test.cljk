(ns crude.phase-test
  ...
  regress on: `:well/lift`/`:production/settle` must NEVER be a member
  of any phase's `:auto` set."
  (:require [clojure.test :refer [deftest is testing]]
            [crude.phase :as phase]))

(deftest well-lift-never-auto-at-any-phase
  (testing "structural invariant: no phase, now or in any future entry, auto-commits a real crude lift"
    (doseq [[n {:keys [auto]}] phase/phases]
      (is (not (contains? auto :well/lift))
          (str "phase " n " must not auto-commit :well/lift")))))
...
(deftest gate-escalates-a-clean-non-auto-write
  (is (= :escalate (:disposition (phase/gate 3 {:op :well/lift} :commit))))
